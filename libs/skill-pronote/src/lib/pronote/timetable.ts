...
import { Course, CourseDay, CourseResource, CourseStatus, CourseType } from '../shared/timetable';
...
const mapCourses = (
  accountId: string,
  courses: (TimetableClassLesson | TimetableClassDetention | TimetableClassActivity)[],
): Course[] => {
...
export async function fetchPronoteCourseResources(
  ...
  const timetableTab = session.user.resources[0].tabs.get(TabLocation.Timetable);
  if (!timetableTab) {
