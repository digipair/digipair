...
export async function fetchPronoteRecipients(session: SessionHandle): Promise<Recipient[]> {
  if (!session) {
    error('Session is undefined', 'fetchPronoteChatMessages');
  }
  ...
  const alLRecipients = await Promise.all(
    session.user.resources.flatMap(resource =>
      [EntityKind.Teacher, EntityKind.Personal].map(kind => newDiscussionRecipients(session, kind)),
    ),
  );
...
function sharedToPronoteRecipient(recipients: Recipient[]): NewDiscussionRecipient[] {
  return recipients
    ...
    .filter((ref): ref is NewDiscussionRecipient => ref !== undefined);
}
