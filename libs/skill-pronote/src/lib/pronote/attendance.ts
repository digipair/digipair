...
import { Absence, Attendance, Delay, Observation, Punishment } from '../shared/attendance';
...
export async function fetchPronoteAttendance(
  ...
  const delays = mapDelays(attendance.delays, accountId).sort(
    (a, b) => a.givenAt.getTime() - b.givenAt.getTime(),
  );
  const absences = mapAbsences(attendance.absences, accountId).sort(
    (a, b) => a.from.getTime() - b.from.getTime(),
  );
  const punishments = mapPunishments(attendance.punishments, accountId).sort(
    (a, b) => a.givenAt.getTime() - b.givenAt.getTime(),
  );
  const observations = mapObservations(attendance.observations).sort(
    (a, b) => a.givenAt.getTime() - b.givenAt.getTime(),
  );
