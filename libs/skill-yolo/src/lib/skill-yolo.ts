...
class YoloService {
   ...
   */
  private async run_model(input: any) {
    const model = await ort.InferenceSession.create(path.dirname(__filename) + '/yolov8m.onnx');
   ...
   */
  private process_output(output: any, img_width: number, img_height: number) {
    let boxes = [] as any[];
   ...
   */
  private union(box1: any, box2: any) {
   ...
   */
  private intersection(box1: any, box2: any) {
    const [box1_x1, box1_y1, box1_x2, box1_y2] = box1;
    const [box2_x1, box2_y1, box2_x2, box2_y2] = box2;
  ...
  async objectDetection(params: any, _pinsSettingsList: PinsSettings[], _context: any) {
    const { image } = params;

    const buf = await this.base64ToImage(image);
    const [input, img_width, img_height] = await this.prepare_input(buf);
    const output = await this.run_model(input);
    const predictions = this.process_output(output, img_width, img_height);
...
export const objectDetection = (params: any, pinsSettingsList: PinsSettings[], context: any) =>
  new YoloService().objectDetection(params, pinsSettingsList, context);
